...
local _option = TS.import(script, game:GetService("ReplicatedStorage"), "TS", "option")
local None = _option.None
local Some = _option.Some
local Ok = TS.import(script, game:GetService("ReplicatedStorage"), "TS", "result").Ok
local RIterator
do
	...
	function RIterator:take(n)
		if n + self.internal_counter >= #self.values then
			local slice = fnutils.slice_array(self.values, n)
